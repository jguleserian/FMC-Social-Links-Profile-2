...
Obviously, I still need improvement updating my BEM skills and understanding. Additionally, I am still trying to develop a "sixth sense" about HTML structure using semantic elements. I structured this page, specifically the links, but creating buttons and converting them to links. As I thought about it more, I kind of wish I had put the links in a `<ul>` and styled each `<li>` as though it were a button. So, I decided to bite the bullet and rewrite that section. It seems like this is more like a page menu, each with links to a social media page. `<buttons>` imply some type of function, not merely a link to another page. The use of and `<ul>` and setting the `<a>` in the `<li>` seemed to remind me of the common structure of menus on a website. So, in the end, I made the `<a>` look like buttons complete with hover and active state changes. If you are interested in taking a look at how I styled everything with buttons, I have retained the original .html and .css files; they are named index2.html and styles2.css. They are properly linked so if you pull up the index2.html, the styles2.css should be linked and show the styles.
